...
// void id_triangle(Vec3i *pts, Vec2i *uvs, TGAImage &image, float intensity, int *zbuffer) {
...
//     for (P.x=bboxmin.x; P.x<=bboxmax.x; P.x++) {
//         for (P.y=bboxmin.y; P.y<=bboxmax.y; P.y++) {
//             Vec3f bc_screen  = i3_barycentric(pts, P);
//             if (bc_screen.x<0 || bc_screen.y<0 || bc_screen.z<0) continue;
//             P.z = 0;
//             for (int i=0; i<3; i++) P.z += pts[i].raw[2]*bc_screen.raw[i];
//             if (zbuffer[int(P.x+P.y*width)]<P.z) {
...
int main(int argc, char** argv) {
    ...
    Projection[3][2] = -1.f/camera.z;
    {
        for (int i=0; i<model->nfaces(); i++) {
            ...
            for (int i=0; i<3; i++) {
                Vec3f pos = model->vert(face[i]);
                pts[i] = world2screen_project_camera(Projection, ViewPort, pos);
                world_coords[i]= pos;
            }
            ...
            if (intensity>0) {
                ...
                id_triangle(pts[0], pts[1], pts[2], uv[0], uv[1], uv[2], image, intensity, zbuffer);
            }
        }
